#include "data/default_background_data.h"
#include "macros.h"

const u32 sTileset_0_Bg_Gfx[949] = INCBIN_U32("data/rooms/Tileset0_Background.gfx.lz");
const u8 sDebug_Bg3[896] = INCBIN_U8("data/rooms/Debug_Bg3.gfx");
const u8 sBackground_Empty[52] = INCBIN_U8("data/rooms/Background_Empty.bin");
const u8 sDoorTransitionTilemap[184] = INCBIN_U8("data/rooms/DoorTransition.tm");
const u8 sBg3_Empty[256] = INCBIN_U8("data/rooms/Bg3_Empty.gfx");
const u8 sDebug_Bg0_0[504] = INCBIN_U8("data/rooms/Debug_Bg0_0.gfx");
