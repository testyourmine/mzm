...
if __name__ == "__main__":
    ...
    parser.add_argument("action", type=str, choices=["rle", "lz", "is_lz", "comp_lz"])
    parser.add_argument("rom_path", type=str, help="Path to input file")
    parser.add_argument("addr", type=str, help="Hex address")
    ...
    if args.action == "rle":
        raw, size = decomp_rle(rom, addr)
        print(f"{len(raw):X}\t{size:X}")
    elif args.action == "lz":
        raw, size = decomp_lz77(rom, addr)
        print(f"{len(raw):X}\t{size:X}")
    elif args.action == "is_lz":
        size = is_lz77(rom, addr)
        print(f"{size:X}")
    elif args.action == "comp_lz":
        raw = comp_lz77(rom, LzCompMethod.MATCHING)
